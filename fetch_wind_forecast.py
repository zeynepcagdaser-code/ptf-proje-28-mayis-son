...
WIND_FORECAST_URL = "https://seffaflik.epias.com.tr/electricity-service/v1/renewables/data/res-generation-and-forecast"

CSV_PATH = "data/wind_forecast.csv"
...
def parse_datetime_columns(df):
    ...
    elif parsed.isna().all() and "hour" in df.columns:
        parsed = pd.to_datetime(
...
def fetch_wind_forecast_data(start_date, end_date, tgt):
    headers = {
    ...
    while current_start <= safe_end_date:
        ...
        print("Rüzgar üretim tahmini çekiliyor:", payload["startDate"], "→", payload["endDate"])
        ...
        if response.status_code != 200:
            ...
            if "geçmiş zaman olmalıdır" in response.text and current_end > datetime.now():
                ...
                print("Rüzgar tahmin endpoint'i ileri endDate kabul etmedi. Güvenli bitişe dönülüyor:", safe_end_date)
...
if os.path.exists(CSV_PATH):
    old_df = pd.read_csv(CSV_PATH)
    print("Eski rüzgar tahmin satır sayısı:", len(old_df))
...
if start_date > end_date:
    print("Yeni çekilecek rüzgar üretim tahmini verisi yok.")
    new_df = pd.DataFrame()
else:
    new_items = fetch_wind_forecast_data(start_date, end_date, tgt)

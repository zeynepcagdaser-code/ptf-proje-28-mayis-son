#!/usr/bin/env python3
"""
Fetch EPİAŞ DAM submitted bid order volume (GÖP Teklif Edilen Alış Miktarları).

Endpoint (electricity-service docs TR 5.132):
  POST /v1/markets/dam/data/submitted-bid-order-volume

Outputs:
  - Raw CSV: data/external/epias_market/dam_microstructure/dam_bid_volume_raw.csv
  - Processed parquet: data/processed/dam_bid_volume.parquet
...
ENDPOINT_URL = "https://seffaflik.epias.com.tr/electricity-service/v1/markets/dam/data/submitted-bid-order-volume"

RAW_DIR = PROJECT_ROOT / "data" / "external" / "epias_market" / "dam_microstructure"
RAW_CSV = RAW_DIR / "dam_bid_volume_raw.csv"
PROCESSED_PATH = PROJECT_ROOT / "data" / "processed" / "dam_bid_volume.parquet"
...
def _normalize_items(items: list[dict[str, Any]]) -> pd.DataFrame:
    if not items:
        return pd.DataFrame(columns=["date", "hour", "bidQuantity"])
    df = pd.DataFrame(items)
    keep = [c for c in ["date", "hour", "bidQuantity"] if c in df.columns]
    out = df[keep].copy()
    out["bidQuantity"] = pd.to_numeric(out.get("bidQuantity"), errors="coerce")
...
def main() -> None:
    ...
    processed = raw[["ts_hour", "bidQuantity"]].rename(columns={"bidQuantity": "dam_bid_volume_mwh"})

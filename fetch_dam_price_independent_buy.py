#!/usr/bin/env python3
"""
Fetch EPİAŞ DAM price-independent bid volumes (FİBA).

Endpoint (electricity-service docs):
  POST /v1/markets/dam/data/price-independent-bid

Outputs:
  - Raw CSV: data/external/epias_market/fiba_fibs/dam_price_independent_buy_raw.csv
  - Processed parquet: data/processed/dam_price_independent_buy.parquet
...
ENDPOINT_URL = "https://seffaflik.epias.com.tr/electricity-service/v1/markets/dam/data/price-independent-bid"

RAW_DIR = PROJECT_ROOT / "data" / "external" / "epias_market" / "fiba_fibs"
RAW_CSV = RAW_DIR / "dam_price_independent_buy_raw.csv"
PROCESSED_PATH = PROJECT_ROOT / "data" / "processed" / "dam_price_independent_buy.parquet"
...
def _master_date_range() -> tuple[datetime, datetime]:
    ...
    if pd.isna(start) or pd.isna(end):
        return datetime(2020, 1, 1), datetime.now()
    # strip tz for payload formatting
...
def _normalize_items(items: list[dict[str, Any]]) -> pd.DataFrame:
    if not items:
        return pd.DataFrame(columns=["date", "hour", "bidVolume"])
    df = pd.DataFrame(items)
    # Keep expected keys if present
    keep = [c for c in ["date", "hour", "bidVolume"] if c in df.columns]
    out = df[keep].copy()
    if "bidVolume" in out.columns:
        out["bidVolume"] = pd.to_numeric(out["bidVolume"], errors="coerce")
    # derive ts_hour in Europe/Istanbul via cleaning util
    ...
    out = out.dropna(subset=["ts_hour"]).sort_values("ts_hour").reset_index(drop=True)
    return out
...
def main() -> None:
    ...
    processed = raw[["ts_hour", "bidVolume"]].copy() if not raw.empty else pd.DataFrame(columns=["ts_hour", "bidVolume"])
    processed = processed.rename(columns={"bidVolume": "dam_price_independent_buy_mwh"})
    processed = processed.drop_duplicates(subset=["ts_hour"], keep="last").sort_values("ts_hour").reset_index(drop=True)

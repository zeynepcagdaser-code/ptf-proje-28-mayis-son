...
LOAD_FORECAST_URL = "https://seffaflik.epias.com.tr/electricity-service/v1/consumption/data/load-estimation-plan"

CSV_PATH = "data/load_forecast.csv"
...
def fetch_load_forecast_data(start_date, end_date, tgt):
    headers = {
    ...
    while current_start <= safe_end_date:
        ...
        print("Yük tahmin planı çekiliyor:", payload["startDate"], "→", payload["endDate"])
        ...
        if response.status_code != 200:
            ...
            if "geçmiş zaman olmalıdır" in response.text and current_end > datetime.now():
                ...
                print("Yük tahmin endpoint'i ileri endDate kabul etmedi. Güvenli bitişe dönülüyor:", safe_end_date)
...
if os.path.exists(CSV_PATH):
    old_df = pd.read_csv(CSV_PATH)
    print("Eski yük tahmin satır sayısı:", len(old_df))
...
if start_date > end_date:
    print("Yeni çekilecek yük tahmin planı verisi yok.")
    new_df = pd.DataFrame()
else:
    new_items = fetch_load_forecast_data(start_date, end_date, tgt)

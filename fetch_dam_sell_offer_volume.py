#!/usr/bin/env python3
"""
Fetch EPİAŞ DAM submitted sales order volume (GÖP Teklif Edilen Satış Miktarları).

Endpoint (electricity-service docs TR 5.133):
  POST /v1/markets/dam/data/submitted-sales-order-volume

Outputs:
  - Raw CSV: data/external/epias_market/dam_microstructure/dam_sell_offer_volume_raw.csv
  - Processed parquet: data/processed/dam_sell_offer_volume.parquet
...
ENDPOINT_URL = "https://seffaflik.epias.com.tr/electricity-service/v1/markets/dam/data/submitted-sales-order-volume"

RAW_DIR = PROJECT_ROOT / "data" / "external" / "epias_market" / "dam_microstructure"
RAW_CSV = RAW_DIR / "dam_sell_offer_volume_raw.csv"
PROCESSED_PATH = PROJECT_ROOT / "data" / "processed" / "dam_sell_offer_volume.parquet"
...
def _normalize_items(items: list[dict[str, Any]]) -> pd.DataFrame:
    if not items:
        return pd.DataFrame(columns=["date", "hour", "offerQuantity"])
    df = pd.DataFrame(items)
    keep = [c for c in ["date", "hour", "offerQuantity"] if c in df.columns]
    out = df[keep].copy()
    out["offerQuantity"] = pd.to_numeric(out.get("offerQuantity"), errors="coerce")
...
def main() -> None:
    ...
    processed = raw[["ts_hour", "offerQuantity"]].rename(columns={"offerQuantity": "dam_sell_offer_volume_mwh"})

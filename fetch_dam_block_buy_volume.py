#!/usr/bin/env python3
"""
Fetch EPİAŞ DAM amount-of-block-buying (GÖP Blok Alış Miktarı).

Endpoint (electricity-service docs TR 5.118):
  POST /v1/markets/dam/data/amount-of-block-buying

Response fields (TR schema 6.14):
  - amountOfPurchasingTowardsMatchedBlock
  - amountOfPurchasingTowardsUnMatchedBlock

Outputs:
  - Raw CSV: data/external/epias_market/dam_microstructure/dam_block_buy_volume_raw.csv
  - Processed parquet: data/processed/dam_block_buy_volume.parquet
...
ENDPOINT_URL = "https://seffaflik.epias.com.tr/electricity-service/v1/markets/dam/data/amount-of-block-buying"

RAW_DIR = PROJECT_ROOT / "data" / "external" / "epias_market" / "dam_microstructure"
RAW_CSV = RAW_DIR / "dam_block_buy_volume_raw.csv"
PROCESSED_PATH = PROJECT_ROOT / "data" / "processed" / "dam_block_buy_volume.parquet"
...
def _normalize_items(items: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    out = df[keep].copy()
    out["amountOfPurchasingTowardsMatchedBlock"] = pd.to_numeric(
        out.get("amountOfPurchasingTowardsMatchedBlock"), errors="coerce"
    )
    out["amountOfPurchasingTowardsUnMatchedBlock"] = pd.to_numeric(
        out.get("amountOfPurchasingTowardsUnMatchedBlock"), errors="coerce"
    )

    # Convert time -> hour string so we can reuse make_ts_hour(date_col=...)
    # time is a string (hour information) in docs.
...
def main() -> None:
    ...
    processed = raw[
        ...
        columns={
            "amountOfPurchasingTowardsMatchedBlock": "dam_block_matched_buy_mwh",
            "amountOfPurchasingTowardsUnMatchedBlock": "dam_block_unmatched_buy_mwh",
        }
    )

#!/usr/bin/env python3
"""
Fetch EPİAŞ DAM price-independent offer volumes (FİBS).

Endpoint (electricity-service docs):
  POST /v1/markets/dam/data/price-independent-offer

Outputs:
  - Raw CSV: data/external/epias_market/fiba_fibs/dam_price_independent_sell_raw.csv
  - Processed parquet: data/processed/dam_price_independent_sell.parquet
...
ENDPOINT_URL = "https://seffaflik.epias.com.tr/electricity-service/v1/markets/dam/data/price-independent-offer"

RAW_DIR = PROJECT_ROOT / "data" / "external" / "epias_market" / "fiba_fibs"
RAW_CSV = RAW_DIR / "dam_price_independent_sell_raw.csv"
PROCESSED_PATH = PROJECT_ROOT / "data" / "processed" / "dam_price_independent_sell.parquet"
...
def _normalize_items(items: list[dict[str, Any]]) -> pd.DataFrame:
    if not items:
        return pd.DataFrame(columns=["date", "hour", "offerVolume"])
    df = pd.DataFrame(items)
    keep = [c for c in ["date", "hour", "offerVolume"] if c in df.columns]
    out = df[keep].copy()
    if "offerVolume" in out.columns:
        out["offerVolume"] = pd.to_numeric(out["offerVolume"], errors="coerce")
...
def main() -> None:
    ...
    processed = raw[["ts_hour", "offerVolume"]].copy() if not raw.empty else pd.DataFrame(columns=["ts_hour", "offerVolume"])
    processed = processed.rename(columns={"offerVolume": "dam_price_independent_sell_mwh"})
